...
export const hasDevanagariText = (value) => DEVANAGARI_RE.test(String(value || ""));

export const isHindiLanguage = (language) => String(language || "").toLowerCase().startsWith("hi");

export const transliterateToHindi = (value) => {
  const text = String(value || "").trim();
  if (!text) return "";
  return text.replace(LATIN_WORD_RE, (word) => transliterateWord(word));
};

export const transliterateHindiToLatin = (value) => {
  const text = String(value || "").trim();
  if (!text) return "";
